...
class REST_API
{
    ...
    public function register_routes(): void
    {
        ...
        register_rest_route($this->namespace, '/languages', [
            'methods' => 'GET',
            'callback' => [$this, 'languages_endpoint'],
    ...
    public function translate_endpoint(\WP_REST_Request $request)
    {
        ...
        if (!$text || !$target_lang) {
            return new \WP_Error('missing_params', 'Text and target_lang are required', ['status' => 400]);
        }
        ...
        if (isset($result['error'])) {
            return new \WP_Error('translation_failed', $result['error'], ['status' => 500]);
        }
