...
function wpste_activate(): void
{
    ...
    // Add default options
    add_option('wpste_settings', [
        'default_language' => 'en',
        'enabled_languages' => ['en'], // Start with English only, user can add more
...
function wpste_run(): void
{
    ...
    // Schedule cron jobs
    add_action('wpste_validate_license', function () {
        $license_manager = new WPSTE\Licensing\License_Manager();
        $license_manager->validate();
    });

    add_action('wpste_cleanup_quota_alerts', function () use ($license_storage) {
    ...
    if (is_admin()) {
        ...
        $quota_notifier = new WPSTE\Licensing\Quota_Notifier($license_storage);
        add_action('admin_notices', [$quota_notifier, 'display_admin_notices']);
    }
