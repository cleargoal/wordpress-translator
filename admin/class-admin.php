...
class Admin {
	...
	public function add_admin_menu(): void {
		add_menu_page(
			__( 'WP Translation Engine', 'smart-translation-engine' ),
			__( 'Translation', 'smart-translation-engine' ),
			'manage_options',
			'wpste-settings',
		...
		add_submenu_page(
			'wpste-settings',
			__( 'Settings', 'smart-translation-engine' ),
			__( 'Settings', 'smart-translation-engine' ),
			'manage_options',
			'wpste-settings',
			array( $this, 'render_settings_page' )
		...
		add_submenu_page(
			'wpste-settings',
			__( 'API Keys', 'smart-translation-engine' ),
			__( 'API Keys', 'smart-translation-engine' ),
			'manage_options',
			'wpste-keys',
		...
		add_submenu_page(
			'wpste-settings',
			__( 'Upgrade', 'smart-translation-engine' ),
			__( 'Upgrade', 'smart-translation-engine' ),
			'manage_options',
			'wpste-upgrade',
	...
	public function render_translation_metabox( $post ): void {
		...
		echo '<p><strong>' . esc_html__( 'Source Language:', 'smart-translation-engine' ) . '</strong> ' . esc_html( strtoupper( $current_lang ) ) . '</p>';

		// Show existing translations
		if ( ! empty( $translations ) ) {
			echo '<div style="margin: 15px 0; padding: 10px; background: #f0f0f1; border-left: 3px solid #2271b1;">';
			echo '<strong>' . esc_html__( 'Existing Translations:', 'smart-translation-engine' ) . '</strong>';
		...
		if ( empty( $available_langs ) ) {
			echo '<p style="color: #666;"><em>' . esc_html__( 'All enabled languages have been translated.', 'smart-translation-engine' ) . '</em></p>';
		} else {
			echo '<p><label for="wpste_target_lang">' . esc_html__( 'Translate to:', 'smart-translation-engine' ) . '</label>';
			echo '<select id="wpste_target_lang" name="wpste_target_lang">';
			echo '<option value="">' . esc_html__( '-- Select Language --', 'smart-translation-engine' ) . '</option>';
			...
			echo esc_html__( 'Translate', 'smart-translation-engine' );
			echo '</button></p>';
		}
		...
		if ( $hook === 'term.php' || $hook === 'edit-tags.php' ) {
			...
			wp_localize_script(
				...
				array(
					...
					'strings'  => array(
						'select_language' => __( 'Please select a target language.', 'smart-translation-engine' ),
						'success'         => __( 'Success!', 'smart-translation-engine' ),
						'error'           => __( 'Error:', 'smart-translation-engine' ),
						'confirm_delete'  => __( 'Are you sure you want to delete this translation?', 'smart-translation-engine' ),
						'deleting'        => __( 'Deleting...', 'smart-translation-engine' ),
						'delete'          => __( 'Delete', 'smart-translation-engine' ),
						'delete_error'    => __( 'Failed to delete translation.', 'smart-translation-engine' ),
					),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Term translated successfully: %s', 'smart-translation-engine' ),
	...
	public function ajax_delete_term_translation(): void {
		...
		if ( ! $deleted ) {
			wp_send_json_error( array( 'message' => __( 'Failed to delete translation.', 'smart-translation-engine' ) ) );
		}

		wp_send_json_success( array( 'message' => __( 'Translation deleted successfully.', 'smart-translation-engine' ) ) );
	}
}
