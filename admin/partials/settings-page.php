...
if (isset($_POST['wpste_settings_submit']) && check_admin_referer('wpste_settings', 'wpste_settings_nonce')) {
    ...
    $settings = [
        ...
        'fallback_providers'  => array_map('sanitize_text_field', $_POST['fallback_providers'] ?? []),
...
// Check current tier
$license = get_option('wpste_license', ['tier' => 'free']);
        ...
        <table class="form-table">
                    ...
                    <?php foreach ($available_providers as $provider): ?>
                        <label>
                            <input type="checkbox" name="fallback_providers[]" value="<?php echo esc_attr($provider); ?>"
                                <?php checked(in_array($provider, $settings['fallback_providers'] ?? [])); ?>>
                            <?php echo esc_html(ucfirst($provider)); ?>
                        </label><br>
                    <?php endforeach; ?>
                    <p class="description">
            ...
            <tr>
                ...
                <td>
                    ...
                    $enabled = $settings['enabled_languages'] ?? ['en', 'uk', 'de'];
                    ...
                    <?php foreach (get_post_types(['public' => true], 'objects') as $post_type): ?>
                        <label>
                            <input type="checkbox" name="post_types[]" value="<?php echo esc_attr($post_type->name); ?>"
                                <?php checked(in_array($post_type->name, $settings['post_types'] ?? ['post', 'page'])); ?>>
